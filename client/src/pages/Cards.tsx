...
function formatCard(card: Card) {
  ...
  return {
    ...
    approvedBy: card.approvedBy ?? undefined,
    transactionLimit: card.transactionLimit ?? undefined,
    dailyLimit: card.dailyLimit ?? undefined,
    monthlyLimit: card.monthlyLimit ?? undefined,
    validFrom: card.validFrom ? new Date(card.validFrom).toISOString().split('T')[0] : undefined,
    validUntil: card.validUntil ? new Date(card.validUntil).toISOString().split('T')[0] : undefined,
    allowedMerchants: card.allowedMerchants ?? undefined,
    allowedMccCodes: card.allowedMccCodes ?? undefined,
    allowedCountries: card.allowedCountries ?? undefined,
    channelRestriction: card.channelRestriction ?? undefined,
    glAccountTemplate: card.glAccountTemplate ?? undefined,
    departmentTemplate: card.departmentTemplate ?? undefined,
    costCenterTemplate: card.costCenterTemplate ?? undefined,
    isOneTimeUse: card.isOneTimeUse ?? undefined,
